...
class Manage
{
	...
	function CheckLogin()
	{
		...
		if (count($results) > 5)
		{
			exitWithErrorPage('System lockout from multiple failed login', 'Please wait for 20 minutes before trying to <a href="/manage_page.php">login</a> again', true);
		}
		else
		{
						...
						<form method="post" action="?action=templates">
							...
							<table class="table table-full table-post">
								<tr>
									...
									<td>
										<textarea class="input input-block" id="templatedata" name="templatedata">' . (htmlspecialchars(file_get_contents(KU_TEMPLATEDIR . '/' . $file))) . '</textarea>
									</td>
								</tr>
								<tr>
			...
			<form method="post" action="?action=news&act=' . $formval . '">
				...
				<table class="table table-post">
					...
					<tr>
						...
						<td>
							<textarea id="news" name="news" class="input input-block" required>' . (isset($values['message']) ? htmlspecialchars($values['message']) : '') . '</textarea>
						</td>
					</tr>
					<tr>
						<td class="text-right"><label for="email">E-mail:</label></td>
						<td>
							<input type="text" id="email" class="input input-block" name="email" value="' . (isset($values['postedemail']) ? $values['postedemail'] : '') . '">
						</td>
					</tr>
					<tr>
						<td class="text-center" colspan="2">
			...
			<form method="post" action="?action=faq&act=' . $formval . '">
				...
				<table class="table table-full table-post">
					<tr>
						...
						<td>
							<input type="text" id="heading" class="input input-block" name="heading" value="' . (isset($values['subject']) ? $values['subject'] : '') . '" required autofocus>
						</td>
					</tr>
					<tr>
						<td class="text-right"><label class="label-required" for="faq">Post:</label></td>
						<td>
							<textarea id="faq" name="faq" class="input input-block" required>' . (isset($values['message']) ? htmlspecialchars($values['message']) : '') . '</textarea>
						</td>
					</tr>
					<tr>
						<td class="text-right">
						...
						<td>
							<input type="text" id="order" class="input input-block" name="order" value="' . (isset($values['order']) ? $values['order'] : '') . '">
						</td>
					</tr>
					<tr>
						<td class="text-center" colspan="2">
			...
			<form method="post" action="?action=rules&act=' . $formval . '">
				...
				<table class="table table-full table-post">
					...
					<tr>
						...
						<td>
							<textarea id="rules" name="rules" class="input input-block" required>' . (isset($values['message']) ? htmlspecialchars($values['message']) : '') . '</textarea>
						</td>
					</tr>
					<tr>
						<td class="text-right">
			...
			<form action="?action=blotter&act=' . $act . '" method="post">
				...
				<table class="table table-post">
					...
					<tr>
						<td class="text-center" colspan="2">
							<label class="btn btn-lg">
								<input type="checkbox" id="important" name="important" ' . ((isset($values['important']) && $values['important'] == 1) ? 'checked' : '') . '>
								Important Blotter?
							</label>
						</td>
					</tr>
				...
				foreach ($results as $line)
				{
					...
					$tpl_page .= '
						<tr>
							...
							<td>' . (strlen($message) > 100 ? substr($message, 0, 100) . '...' : $message) . '</td>
							<td>' . (($line['important'] == 1) ? '<i class="icon icon-ok"></i> Yes' : '<i class="icon icon-remove"></i> No') . '</td>
							<td>
			...
			if (count($results) > 0)
			{
				if (isset($_POST['submitting']))
				{
					...
					else
					{
						...
						foreach ($results as $line)
						{
							$boards = array_merge($boards, array(
								$line['name']
							));
						}
						...
						while (list($postkey, $postvalue) = each($_POST))
						{
							...
							{
								$changed_boards = array_merge($changed_boards, array(
									substr($postkey, 8)
								));
							}
						}
						while (list(, $thisboard_name) = each($boards))
						{
							...
							{
								$newboards = array_merge($newboards, array(
									$thisboard_name
								));
							}
						}
							...
							<tr>
								...
								<td>
									<label class="btn text-bold">
										<input type="checkbox" name="modsallboards" ' . ($boards == array(
					'allboards'
				) ? 'checked' : '') . '>
										All Boards
									</label>
				...
				if (count($results) > 0)
				{
					if (!isset($_POST['replacedby']))
					{
						...
						foreach ($results as $line)
						{
							...
							{
								$array_boards = array_merge($array_boards, array(
									$lineboard['name']
								));
							}
							foreach ($array_boards as $this_board_name)
							{
								$tpl_page .= '
									<label class="btn">
										<input type="checkbox" name="wordfilter[]" value="' . $this_board_name . '" ' . (in_array($this_board_name, $existingboards) ? 'checked' : '') . '>
										/' . $this_board_name . '/
									</label>
								';
							}
			...
			if (count($resultsboard) > 0)
			{
				foreach ($resultsboard as $lineboard)
				{
					...
					foreach ($embeds as $embed)
					{
						$tpl_page .= '
							<label class="btn">
								<input type="checkbox" name="allowedembeds[]" value="' . $embed['filetype'] . '" ' . (in_array($embed['filetype'], $embedsAllowed) ? 'checked' : '') . '>
								' . $embed['name'] . '
							</label>
						';
					}
	...
	function BoardList($username)
	{
		...
		if ($this->CurrentUserIsAdministrator() || $results[0][0] == 'allboards')
		{
			...
			foreach ($resultsboard as $lineboard)
			{
				$staff_boardsmoderated = array_merge($staff_boardsmoderated, array(
					array(
						'name' => $lineboard['name'],
						'id' => $lineboard['id']
					)
	...
	function spam()
	{
		...
		$content = file_get_contents(KU_ROOTDIR . 'spam.txt');
