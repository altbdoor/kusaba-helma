...
class Parse
{
	...
	function BBCode($string)
	{
		...
		$replaces = array(
			'<span class="post-comment-bold">\\1</span>',
			'<span class="post-comment-italic">\\1</span>',
			'<span class="post-comment-underline">\\1</span>',
			'<span class="post-comment-strike">\\1</span>',
			'<div class="post-comment-ascii">\\1</div>',
			'<span class="post-comment-spoiler">\\1</span>'
		);
		$string = preg_replace($patterns, $replaces, $string);
		$string = preg_replace_callback('`\[code\](.+?)\[/code\]`is', array(
			&$this,
			'code_callback'
		), $string);
	...
	{
		$return = '<div class="post-comment-code">' . str_replace('<br />', '', $matches[1]) . '</div>';
	...
	function ClickableQuote($buffer, $board, $boardtype, $parentid, $boardid, $ispage = false)
	{
		...
		$buffer = preg_replace_callback('/&gt;&gt;([r]?[l]?[f]?[q]?[0-9,\-,\,]+)/', array(
			&$this,
			'InterthreadQuoteCheck'
		), $buffer);
		
		/* Add html for links to posts made in a different board */
		$buffer = preg_replace_callback('/&gt;&gt;\/([a-z]+)\/([0-9]+)/', array(
			&$this,
			'InterboardQuoteCheck'
		), $buffer);
