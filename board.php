...
if ($posting_class->CheckValidPost($is_oekaki))
{
	...
	if ($board_class->board['locked'] == 0 || ($user_authority > 0 && $user_authority != 3))
	{
		...
		if ((file_exists(KU_BOARDSDIR . $board_class->board['name'] . '/src/' . $upload_class->file_name . $upload_class->file_type) && file_exists(KU_BOARDSDIR . $board_class->board['name'] . '/thumb/' . $upload_class->file_name . 's' . $upload_class->file_type)) || ($file_is_special && file_exists(KU_BOARDSDIR . $board_class->board['name'] . '/src/' . $upload_class->file_name . $upload_class->file_type)) || $post_fileused == false)
		{
			...
			if (strtolower($_POST['em']) != 'sage' && $ords_email != array(
				19979,
				12370
			) && strtolower($_POST['em']) != 'age' && $ords_email != array(
				19978,
				12370
			) && $_POST['em'] != 'return' && $_POST['em'] != 'noko')
			{
			...
			// we'll have localstorage for that
		...
		if ($thread_replyto != '0' && strtolower($_POST['em']) != 'sage' && unistr_to_ords($_POST['em']) != array(
			19979,
			12370
		))
		{
		...
		if (isset($_POST['reportpost']))
		{
			...
			if ($board_class->board['enablereporting'] == 1)
			{
				...
				if ($post_reported === 'cleared')
				{
					...
					$resultArray[$val] = _gettext('That post has been cleared as not requiring any deletion.');
				}
				elseif ($post_reported)
				{
					//echo _gettext('That post is already in the report list.') . '<br />';
					$resultArray[$val] = _gettext('That post is already in the report list.');
				}
				else
				{
					if ($post_class->Report())
					{
						...
						$resultArray[$val] = _gettext('Post successfully reported.');
					}
					else
					{
						//echo _gettext('Unable to report post. Please go back and try again.') . '<br />';
						$resultArray[$val] = _gettext('Unable to report post. Please go back and try again.');
					}
				}
			}
			else
			{
				...
				$resultArray[$val] = _gettext('This board does not allow post reporting.');
			}
		}
		elseif (
			isset($_POST['postpassword']) || (
				isset($_POST['moddelete']) &&
				$manageClass->CurrentUserIsModeratorOfBoard($board_class->board['name'], $_SESSION['manageusername']) &&
				$ismod = true
			)
		)
		{
			...
			if ($_POST['postpassword'] != '' || $ismod)
			{
				if (md5($_POST['postpassword']) == $post_class->post['password'] || $ismod)
				{
					if (isset($_POST['fileonly']))
					{
						if ($post_class->post['file'] != '' && $post_class->post['file'] != 'removed')
						{
							...
							$resultArray[$val] = _gettext('Image successfully deleted from your post.');
						}
						else
						{
							//echo _gettext('Your post already doesn\'t have an image!') . '<br />';
							$resultArray[$val] = _gettext('Your post already doesn\'t have an image!');
						}
					}
					else
					{
						if ($post_class->Delete())
						{
							...
							$resultArray[$val] = _gettext('Post successfully deleted.');
						}
						else
						{
							//echo _gettext('There was an error in trying to delete your post') . '<br />';
							$resultArray[$val] = _gettext('There was an error in trying to delete your post');
						}
					}
				}
				else
				{
					...
					$resultArray[$val] = _gettext('Incorrect password.');
				}
			}
			else
			{
				//do_redirect(KU_BOARDSPATH . '/' . $board_class->board['name'] . '/');
				$resultArray[$val] = 'Invalid action';
			}
		}
	}
